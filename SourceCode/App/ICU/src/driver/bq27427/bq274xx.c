...
#define BQ27427_SUBCLASS_DELAY K_MSEC(5)
#define BQ27427_COMMAND_WAITING_TIME_USEC (66)
...
#define BQ274XX_INIT_COMP_DELAY K_MSEC(50)
#define BQ274XX_INIT_COMP_MAX_TRIES 100
...
/* Control stauts */
#define BQ27XXX_STATUS_INITCOMP BIT(7)
#define BQ274XX_STATUS_WDTRESET BIT(14)

/* Config update mode flag */
#define BQ27XXX_FLAG_CFGUP BIT(4)
#define BQ27XXX_FLAG_ITPOR BIT(5)
...
static const struct golden_block golden_blocks[] = {
    //!< 7.4.1.1 Safety Subclass (ID - 2)
    [0] = { 0x02, 0x00, {
    ...
    //!< 7.4.1.2 Charge Termination Subclass (ID - 36)
    [1] = { 0x24, 0x00, {
    ...
    //!< 7.4.1.3 Discharge Subclass (ID - 49)
    [2] = { 0x31, 0x00, {
    ...
    //!< 7.4.1.4 Registers (ID - 64)
    [3] = { 0x40, 0x00, {
            0x64, 0x78, 0x0F, 0x9F, 0x2B, 0x00, 0x00, 0x14,
            0x04, 0x00, 0x09, 0x04, 0x27, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
          }},
    //!< Private parameters, Not documented
    [4] = { 0x44, 0x00, {
    ...
    //!< 7.4.2.2 Current Thresholds Subclass [ID - 81]
    [8] = { 0x51, 0x00, {
    ...
    //!< 7.4.2.3 State Subclass [ID - 82]
    /**  Offset  Type  Name               Value
     *   06      I2    Design Capacity    250 (0x00FA)
     *   10      I2    Terminate Voltage  3200 (0x0C80)
     *   21      I2    Taper Rate         100 (0x0064)
     */
    [9] = { 0x52, 0x00, {
            0x4B, 0x44, 0x00, 0x00, 0x00, 0x81, 0x00, 0xFA,
            0x03, 0x9D, 0x0C, 0x80, 0x00, 0xC8, 0x00, 0x32,
            0x00, 0x14, 0x03, 0xE8, 0x01, 0x00, 0x64, 0x00,
            0x0A, 0xFF, 0xCD, 0xFF, 0xCD, 0x00, 0x01, 0x00,
          }},
    //!< 7.4.3.1 R_a RAM Subclass [ID - 89]
    [10]= { 0x59, 0x00, {
            0x00, 0x32, 0x00, 0x1E, 0x00, 0x25, 0x00, 0x39,
            0x00, 0x3A, 0x00, 0x3C, 0x00, 0x50, 0x00, 0x50,
            0x00, 0x5D, 0x00, 0x6C, 0x00, 0x78, 0x00, 0x7F,
            0x00, 0xC5, 0x01, 0x77, 0x02, 0x8D, 0x00, 0x00,
          }},
    //!< Private parameters, Not documented
    [11]= { 0x6D, 0x00, {
            0x09, 0x79, 0x0E, 0xE1, 0x0E, 0xA0, 0x10, 0x67,
            0x10, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
          }},
    //!< 7.4.5.2 CC Cal Subclass [ID - 105]
    [12]= { 0x69, 0x00, {
            0x00, 0x01, 0x0B, 0xCA, 0x7F, 0x24, 0xDD, 0x2F,
            0x93, 0x3B, 0x94, 0x1F, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
          }},
    //!< 7.4.6.1 Codes Subclass (ID - 112)
    [13]= { 0x70, 0x00, {
...
static int bq274xx_get_device_type(const struct device *dev, uint16_t *val)
{
    ...
    //!< 3.3 I2C Command waiting time
    k_usleep(BQ27427_COMMAND_WAITING_TIME_USEC);
...
static int bq274xx_read_block(const struct device *dev, uint8_t subclass, uint8_t *block, uint8_t num_bytes)
{
...
static int bq274xx_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val)
{
    struct bq274xx_data *data = dev->data;
    float int_temp;
...
//!< Custom API defined begin

static bool bq27427_detect_iptor(const struct device *dev)
{
    uint16_t flags;
    int ret;
    ...
    LOG_INF("detect iptor, flags: 0x%04X", flags);

    return !!(flags & BQ27XXX_FLAG_ITPOR);
}

static int bq27427_set_chemistry(const struct device *dev)
{
    int ret;

    ret = bq274xx_ctrl_reg_write(dev, BQ27427_CTRL_CHEM_B);
    if (ret < 0) {
        LOG_ERR("Unable to configure chemistry");
        return -EIO;
    }
...
static int bq27427_get_chemistry(const struct device *dev)
{
    int ret;
    uint16_t val;
    ...
    LOG_INF("Chem ID: 0x%04X", val);

    return 0;
}

static int bq27427_verify_firmware_version(const struct device *dev)
{
    int ret;
    uint16_t val;

    ret = bq274xx_get_device_type(dev, &val);
    if (ret < 0) {
        LOG_ERR("Unable to get device ID");
        return -EIO;
    }

    if (val == BQ27427_DEVICE_ID) {
        LOG_INF("Device ID: 0x%04X", val);
    } else {
        LOG_ERR("Unsupported device ID: 0x%04x", val);
        return -ENOTSUP;
    }

    ret = bq274xx_ctrl_reg_write(dev, BQ274XX_CTRL_FW_VERSION);
    if (ret < 0) {
        LOG_ERR("Unable to write control register");
        return -EIO;
    }
    ...
    LOG_INF("FW version: 0x%04X", val);

    return 0;
}

static int bq27427_write_block(const struct device *dev, uint8_t subclass, uint8_t offset, const uint8_t *block, uint8_t num_bytes)
{
    const struct bq274xx_config *const config = dev->config;
    uint8_t checksum = 0;
    int ret;
    ...
    ret = i2c_reg_write_byte_dt(&config->i2c, BQ274XX_EXT_DATA_BLOCK, offset);
    if (ret < 0) {
        LOG_ERR("Failed to update block offset");
        return -EIO;
    }
    k_sleep(BQ27427_SUBCLASS_DELAY);
    ...
    uint8_t val;
    ret = i2c_reg_read_byte_dt(&config->i2c, BQ274XX_EXT_CHECKSUM, &val);
    if (ret < 0) {
        LOG_ERR("Failed to read block checksum");
        return -EIO;
    }
    ...
    if (checksum == val) {
        LOG_INF("Checksum Compare successe! [0x%0X]", checksum);
    }
    else {
        LOG_ERR("Checksum Compare failed! [0x%0X] - [0x%0X]", checksum, val);
        return -EIO;
    }
...
static uint16_t bq27427_get_flags(const struct device *dev)
{
    int ret;
    uint16_t flags;
...
static uint16_t bq27427_get_control_status(const struct device *dev)
{
    int ret;
    uint16_t val;
...
static bool bq27427_detect_wdreset(const struct device *dev)
{
    uint16_t ctr_status;

    ctr_status = bq27427_get_control_status(dev);
    if (ctr_status < 0) {
        LOG_ERR("Unable to read control status");
        return false;
    }

    LOG_INF("detect wdreset, status: 0x%04X", ctr_status);

    return !!(ctr_status & BQ274XX_STATUS_WDTRESET);
}

int bq27427_init_complete(const struct device *dev)
{
    uint16_t status;
    uint8_t try;
    int ret;
    ...
    for (try = 0; try < BQ274XX_INIT_COMP_MAX_TRIES; try++) {
        ...
        initcomp_flag = !!(status & BQ27XXX_STATUS_INITCOMP);
        if (initcomp_flag) {
            LOG_INF("INITCOMP ready, try %u (0x%04X)", try, status);
            break;
        }

        k_sleep(BQ274XX_INIT_COMP_DELAY);
    }

    if (try >= BQ274XX_INIT_COMP_MAX_TRIES) {
...
static int bq27427_load_golden_data(const struct device *dev)
{
    int ret;

    size_t count = sizeof(golden_blocks) / sizeof(golden_blocks[0]);
    for (size_t i = 0; i < count; i++) {
        LOG_DBG("[%d], Flags: 0x%04X", i, bq27427_get_flags(dev));

        ret = bq27427_write_block(dev, golden_blocks[i].subclass, golden_blocks[i].offset, golden_blocks[i].data, BQ27XXX_DM_SZ);
        if (ret < 0) {
            LOG_ERR("load golden data failed at %zu", i);
            return ret;
        }
        LOG_INF("load golden data success at %zu", i);
    }

    return 0;
}

static int bq27427_unseal(const struct device *dev)
{
    int ret;
...
int bq27427_calibrate(const struct device *dev)
{
    struct bq274xx_data *data = dev->data;
    int ret;
    ...
    LOG_DBG("1 CtrlStaus: 0x%04X", bq27427_get_control_status(dev));

    //!< Set CFGUPDATE
    ret = bq274xx_mode_cfgupdate(dev, true);
    if (ret < 0) {
        LOG_ERR("Enter mode cfgupdate failed");
        return ret;
    }

    LOG_DBG("S1, Flags: 0x%04X", bq27427_get_flags(dev));

    ret = bq27427_set_chemistry(dev);
    if (ret < 0) {
        LOG_ERR("Set chemID failed");
        return ret;
    }

    ret = bq27427_verify_firmware_version(dev);
    if (ret < 0) {
        LOG_ERR("Verify FW version failed");
        return ret;
    }

    //!< Load golden data
    ret = bq27427_load_golden_data(dev);
    if (ret < 0) {
        LOG_ERR("Load golden block failed");
        return ret;
    }
    ...
    ret = bq274xx_mode_cfgupdate(dev, false);
    if (ret < 0) {
        LOG_ERR("Exit mode cfgupdate failed");
        return ret;
    }

    LOG_DBG("S2, Flags: 0x%04X", bq27427_get_flags(dev));
    ...
    LOG_DBG("2 CtrlStaus: 0x%04X", bq27427_get_control_status(dev));
...
static int bq274xx_gauge_init(const struct device *dev)
{
    ...
    //!< By checking INTCOMP, it was confirmed that the initialization of bq27427 was completed
    ret = bq27427_init_complete(dev);
    if (ret < 0) {
        LOG_ERR("Device initialize failed");
        return -EIO;
    }
    ...
    bool iptor_enabled = bq27427_detect_iptor(dev);
    bool wdreset_enable = bq27427_detect_wdreset(dev);

    if (!config->lazy_loading) {
        if (iptor_enabled || wdreset_enable) {
            ret = bq27427_calibrate(dev);
            // k_msleep(1500);
            LOG_INF("BQ27427_CALIBRATE (%d) ++++++++++++++++++++++++++++++++++++++++++ ", ret);
        } else {
            ret = bq274xx_gauge_configure(dev);
            LOG_INF("BQ27427_CONFIGURE (%d) ++++++++++++++++++++++++++++++++++++++++++ ", ret);
        }
        bq27427_get_chemistry(dev);
    }
