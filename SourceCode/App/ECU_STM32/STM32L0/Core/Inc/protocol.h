...
#define PDU_DATA_SIZE   64

#define MSG_FRAME_TIMEOUT   100
 ...
 */
enum {
    SDU_SYNC_HEAD_LSB= 0xAA,
    SDU_SYNC_HEAD_MSB = 0x55,
    SDU_SYNC_TAIL = 0xFF,
};
...
typedef enum
{
    FRAME_CHARGESTATE = 0x1E,  //!< ECU charging state notify
    FRAME_OPENICU     = 0x25,  //!< Open/Close ICU
    FRAME_LED         = 0x26, 
} FrameType;
...
#define SDU_HANDLER_NUM  0x80
typedef void (*pdu_handler)(pdu_t *p_pdu);
...
extern void sdu_send(uint8_t type, uint16_t len, uint8_t *data);
extern void key_event_notify(ButtonStatus event);

#endif /* __MIS_PROTOCOL_H__ */
