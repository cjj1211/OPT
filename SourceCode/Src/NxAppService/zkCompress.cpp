 ...
 */
bool zkCompressThread::compressFile(QuaZip* zip, QString fileName, QString fileDest) {
 ...
 * funzione.
 */
bool zkCompressThread::compressSubDir(QuaZip* zip, QString dir, QString origDir, bool recursive) {
	// zip: oggetto dove aggiungere il file
 ...
 */
bool zkCompressThread::extractFile(QuaZip* zip, QString fileName, QString fileDest) {
	// zip: oggetto dove aggiungere il file
	// filename: nome del file reale
	...
	if (!inFile.open(QIODevice::ReadOnly, strMD5.toUtf8().constData()) || inFile.getZipError() != UNZ_OK) return false;
 ...
 * di cancellarli.
 */
bool zkCompressThread::removeFile(QStringList listFile) {
	bool ret = true;
	// Per ogni file
 ...
 */
bool zkCompressThread::compressFile(QString fileCompressed, QString file) {
	// Creo lo zip
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
bool zkCompressThread::compressFiles(QString fileCompressed, QStringList files) {
	// Creo lo zip
	QuaZip zip(fileCompressed);
	QDir().mkpath(QFileInfo(fileCompressed).absolutePath());
	if (!zip.open(QuaZip::mdCreate)) {
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
bool zkCompressThread::compressDir(QString fileCompressed, QString dir, bool recursive) {
	// Creo lo zip
	QuaZip zip(fileCompressed);
	QDir().mkpath(QFileInfo(fileCompressed).absolutePath());
	if (!zip.open(QuaZip::mdCreate)) {
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
QString zkCompressThread::extractFile(QString fileCompressed, QString fileName, QString fileDest) {
	// Apro lo zip
	QuaZip zip(fileCompressed);
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
QStringList zkCompressThread::extractFiles(QString fileCompressed, QStringList files, QString dir) {
	// Creo lo zip
	QuaZip zip(fileCompressed);
	if (!zip.open(QuaZip::mdUnzip)) {
		return QStringList();
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
QStringList zkCompressThread::extractDir(QString fileCompressed, QString dir) {
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
QStringList zkCompressThread::getFileList(QString fileCompressed) {
	// Apro lo zip
...
void zkCompressThread::run()
{
	if (m_iType == compress_Dir)
	{
		compressDir(m_pFileCompressed, m_pSelectPath);
	}
	if (m_iType == extract_Dir)
	{
		extractDir(m_pFileCompressed, m_pSelectPath);
	}
}
zkCompressThread::zkCompressThread(zkCompressThread::zkCompressType type, QString fileCompressed, QString selectPath)
{
	m_iType = type;
	m_pFileCompressed = fileCompressed;
	m_pSelectPath = selectPath;
	m_iAllFileCount = 0;
	m_iCompressFileCount = 0;
}
void zkCompressThread::compressProgress(QString fileName)
{
	if (m_iType == compress_Dir)
		emit sig_compressProgress(fileName, ++m_iCompressFileCount, m_iAllFileCount);
	else if (m_iType == extract_Dir)
		emit sig_extractProgress(fileName, ++m_iCompressFileCount, m_iAllFileCount);
}
