...
{
public:
    static CSystemCfg& Instance(const QString& filePath = "../config/SystemConfig.json");
    SystemCfg GetSystemCfg();
    bool UpdateSystemCfg(const SystemCfg& cfg);

private:
    CSystemCfg(const QString& filePath);
    ...
    bool checkSystemCfg(const SystemCfg& cfg) const;
};
