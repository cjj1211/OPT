...
CSystemCfg& CSystemCfg::Instance(const QString& filePath)
{
...
bool CSystemCfg::UpdateSystemCfg(const SystemCfg& cfg)
{
   ...
   /*     jsonFile.write(QByteArray::fromStdString(jsonDoc));*/
        QByteArray jsonData = QByteArray::fromStdString(jsonDoc);
        QJsonDocument jsonDoc = QJsonDocument::fromJson(jsonData);
        QJsonObject jsonObj = jsonDoc.object();
        QJsonDocument outputDoc(jsonObj);
        jsonFile.write(outputDoc.toJson());
        jsonFile.close();
...
bool CSystemCfg::checkSystemCfg(const SystemCfg& cfg) const
{
    ...
    if(systemCfg.Hardware.SaveFileType != "rhd" && systemCfg.Hardware.SaveFileType != "edf")
    {
        LOG_ERR("Configuration file struct Hardware SaveFileType must be 'rhd' or 'edf'");
        return false;
    }
    return true;
}

CSystemCfg::CSystemCfg(const QString& filePath)
{
    ...
    const QByteArray byteArray = configFilePath.toUtf8(); // 使用UTF-8编码将QString转换为QByteArray
    const auto path = std::string(byteArray.constData(), byteArray.length());
