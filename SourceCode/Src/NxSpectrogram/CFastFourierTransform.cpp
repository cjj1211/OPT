...
CFastFourierTransform::CFastFourierTransform(float sampleRate_, unsigned int length_, WindowFunction function_) :
...
void CFastFourierTransform::setLength(int length_)
{
    length = length_;
    createWindow();
    createPsdVector();
...
void CFastFourierTransform::createWindow()
{
    if (window) delete[] window;
...
void CFastFourierTransform::createPsdVector()
{
    if (logPsd) delete[] logPsd;
    logPsd = new float[(length >> 1) + 1];
}

void CFastFourierTransform::createFrequencyVector()
{
    if (frequency) delete[] frequency;
...
void CFastFourierTransform::complexInputFft(float* data, unsigned int n)
{
...
void CFastFourierTransform::realInputFft(float* data, unsigned int n)
{
...
float* CFastFourierTransform::logSqrtPowerSpectralDensity(float* data)
{
    // Apply window.
...
float CFastFourierTransform::getFrequency(int index) const
{
